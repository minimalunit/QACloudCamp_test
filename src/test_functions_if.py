...
def test_check_successful_receiving_id_post():
    ...
    assert get_posts_id_response.status_code == 200, f'something went wrong with getting posts with id {post_id_number}'
    get_posts_id_data = get_posts_id_response.json()
    assert get_posts_id_data['userId'] == post_id_number, f'something went wrong with getting posts with id {post_id_number}'
...
def test_check_successful_delete_post():
    post_posts_response = post_posts()
    post_posts_data = post_posts_response.json()
    delete_id = post_posts_data['id']
    assert delete_posts_id(delete_id).status_code == 200, f'something went wrong with deleting post {delete_id}'
    get_posts_id_response = get_posts_id(delete_id)
    assert get_posts_id_response.status_code == 404, f'the post {delete_id} has not been deleted'
